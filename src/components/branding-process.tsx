...
function scalePathY(d: string, sy: number): string {
  return d.replace(/([\d.]+)\s+([\d.]+)/g, (_, x, y) => `${x} ${parseFloat(y) * sy}`)
}
...
export const BrandingProcess = () => {
  ...
  return (
    ...
    >
      ...
      >
        <Link to="/contact" className="btn-industrial inline-flex items-center gap-3">
...
const Dot = ({ wp, t, progress }: { wp: Waypoint; t: number; progress: MotionValue<number> }) => {
...
const Label = ({ wp, t, progress }: { wp: Waypoint; t: number; progress: MotionValue<number> }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm leading-relaxed text-white/55 md:text-base">{wp.body}</p>
    </motion.div>
  )
}
