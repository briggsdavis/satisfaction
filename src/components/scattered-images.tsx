...
const ScatteredImage = ({
  ...
  const scrollScale = useTransform(scrollProgress, [0, 1], slot.zoomIn ? [1, 1.5] : [1, 0.1])

  const scrollOpacity = useTransform(scrollProgress, [0, 0.8], [1, 0])
  const blurPx = useTransform(scrollProgress, [0, 0.8], [0, 12])
  const blurFilter = useTransform(blurPx, (b: number) => (!slot.zoomIn ? `blur(${b}px)` : "none"))
...
const ResolvedScatteredImage = ({
  ...
  if (!url) return null
  return <ScatteredImage slot={slot} src={url} scrollProgress={scrollProgress} />
}

export function ScatteredImages({ scrollProgress }: { scrollProgress: MotionValue<number> }) {
