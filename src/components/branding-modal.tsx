...
// ─── Field helpers ────────────────────────────────────────────────────────────
type FieldRender = (props: { id: string; "aria-label": string }) => React.ReactNode

const Field = ({ label, children }: { label: string; children: FieldRender }) => {
  const id = useId()
  return (
    <div className="border-b border-white/10 py-5">
      <label
        htmlFor={id}
        className="mb-3 block text-xs font-bold tracking-[0.35em] text-white/40 uppercase"
      >
        {label}
      </label>
      {children({ id, "aria-label": label })}
    </div>
  )
}

// For Field instances whose children are a group (radios/checkboxes), there's
// no single input to bind htmlFor to — use this instead so the section header
// remains a styled label without an unresolved htmlFor.
...
const USAGE_OPTIONS = ["Social Media", "Website", "Print / Packaging", "Signage", "Apparel / Merch"]

const STYLE_OPTIONS = ["Minimal", "Bold", "Classic", "Playful", "Luxury", "Industrial"]

// ─── Branding Modal ───────────────────────────────────────────────────────────
export const BrandingModal = ({ open, onClose }: { open: boolean; onClose: () => void }) => {
  ...
  const toggleUsage = (opt: string) =>
    setUsage((prev) => (prev.includes(opt) ? prev.filter((u) => u !== opt) : [...prev, opt]))
              ...
              <AnimatePresence mode="wait">
                {submitted ? (
                  ...
                  >
                    ...
                    <h3 className="mb-3 text-2xl font-bold uppercase">Brief received</h3>
                    <p className="max-w-sm text-sm leading-relaxed text-white/50">
                      We'll review your brand brief and reach out within 48 hours to kick things
                      off.
                    </p>
                    <button onClick={handleClose} className="btn-industrial mt-10">
                      Close
                    </button>
                  </motion.div>
                ) : (
                  ...
                  >
                    ...
                    <Field label="02 — Type of Business">
                      {(p) => (
                        <select {...p} name="industry" className={selectCls}>
                    ...
                    {/* Q5 — Style direction */}
                    <FieldGroup label="05 — Visual Style Direction">
                    ...
                    {/* Q7 — Usage */}
                    <FieldGroup label="07 — Where Will the Logo Be Used?">
                    ...
                    <Field label="09 — Timeline">
                      {(p) => (
                        <select {...p} name="timeline" className={selectCls}>
                          <option value="">Select…</option>
