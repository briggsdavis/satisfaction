    ...
    <div className="w-full max-w-md overflow-hidden rounded-[16px] border border-white/10 bg-neutral-950">
      ...
      <div className="p-5 font-mono text-sm">
        {lines.map((line, i) => (
          ...
          >
            <span className={line.accent ? "text-cyan-400" : "text-white/25"}>{line.label}</span>
            <span className={i === lines.length - 1 ? "text-green-400" : "text-white/65"}>
              {line.text}
            </span>
          </motion.div>
        ))}
...
const PrototypeVisual = () => (
  <div className="w-full max-w-md overflow-hidden rounded-[16px] border border-white/10 bg-neutral-950">
    ...
    <div className="space-y-2 p-4">
      ...
      >
        <span className="font-mono text-xs text-cyan-400">prototype_ready.fig ✓</span>
      </motion.div>
    </div>
  </div>
)
      ...
      {rounds.map((round, i) => (
        ...
        >
          <div className="relative">
            ...
            >
              <span className="font-mono text-xl font-bold text-white">0{i + 1}</span>
            </motion.div>
          </div>
          <motion.span
...
// ─── Visuals map ─────────────────────────────────────────────────────────────
const VISUALS = [DiscoveryVisual, ScopeVisual, PrototypeVisual, RevisionVisual, HandOffVisual]

// ─── Main component ───────────────────────────────────────────────────────────
export const WebDevProcess = () => {
  const wrapperRef = useRef<HTMLDivElement>(null)
  const { pinY, progress, pinDistance } = usePinnedScroll(wrapperRef, () => window.innerHeight * 5)
    ...
    <div ref={wrapperRef} style={{ height: `calc(${pinDistance}px + 100vh)` }} className="relative">
      ...
      >
        ...
        <div className="pointer-events-none absolute top-8 right-8 z-10 md:top-12 md:right-14">
          <p className="font-mono text-xs tracking-[0.3em] text-white/25 uppercase">process</p>
        </div>
          ...
          {STEPS.map((_, i) => (
            <motion.div
              ...
              animate={{
                ...
                backgroundColor:
                  stepIndex === i ? "rgba(34,211,238,0.85)" : "rgba(255,255,255,0.18)",
              }}
