...
const ProjectImage = ({ storageId }: { storageId: Id<"_storage"> | undefined }) => {
...
const ProjectCard = ({ project, category }: { project: Project; category?: Category }) => {
  const href = category ? `/portfolio/${category.slug}/${project.slug}` : `/portfolio`
  return (
    <Link
      to={href}
...
const Group = ({
  ...
  if (kind === "tall") {
    return (
      <motion.div className="h-full w-[260px] shrink-0 md:w-[280px]" {...animProps(delayStart)}>
  ...
  return (
    <div className="flex h-full shrink-0 flex-col gap-4" style={{ width: "556px" }}>
      <motion.div className="flex-1" {...animProps(delayStart)}>
        <ProjectCard project={projects[0]} category={cat(projects[0])} />
      </motion.div>
...
export const FeaturedCascade = () => {
  ...
  const categoryById = useMemo(
    () => new Map((categories ?? []).map((c) => [c._id, c] as const)),
    [categories],
  )

  const scrollRef = useRef<HTMLElement>(null)
  useDragScroll(scrollRef, { speed: 1.5 })
  ...
  return (
    <section className="bg-black pt-12 pb-20">
      ...
      >
        <div className="flex gap-4" style={{ width: "max-content", height: "580px" }}>
          {groups.map((g, gi) => (
            <Group
              key={gi}
              kind={g.kind}
