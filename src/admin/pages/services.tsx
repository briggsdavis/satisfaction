    ...
    <div className="border border-white/10">
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          />
          <span className="truncate text-sm font-bold">{service.name || "Untitled Service"}</span>
      ...
      {open && (
        <div className="border-t border-white/10 px-4 pb-4">
          ...
          <div className="border-b border-white/10 py-4">
            <label
              htmlFor={`service-color-${service._id}`}
          ...
          <div className="border-b border-white/10 py-4">
            <p className="mb-2 text-xs font-bold tracking-[0.35em] text-white/40 uppercase">Size</p>
...
export const ServicesAdmin = () => {
  ...
  const handleAddFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const maxOrder = (services ?? []).reduce((m, s) => Math.max(m, s.order), -1)
      await create({
