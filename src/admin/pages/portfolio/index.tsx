        ...
        {(categories ?? []).map((cat) => (
          ...
          >
            ...
            >
              ...
              />
              <span className="truncate text-sm font-bold">{cat.name || "Untitled"}</span>
              <span className="font-mono text-xs text-white/30">/{cat.slug}</span>
            </Link>
            <button
              onClick={() => {
                if (confirm(`Delete category "${cat.name}"?`)) remove({ id: cat._id })
              }}
