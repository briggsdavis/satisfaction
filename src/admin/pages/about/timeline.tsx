    ...
    <div className="border border-white/10">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-2.5">
        <div className="min-w-0">
          <span className="block truncate text-sm font-bold">{item.client || "Untitled"}</span>
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() => {
              if (confirm(`Delete "${item.client || "this entry"}"?`)) remove({ id: item._id })
            }}
      ...
      <div className="px-4 pb-2">
        ...
        <ConvexTextField
          ...
          value={item.campaign ?? ""}
          onCommit={(v) => update({ id: item._id, campaign: v.trim() || undefined })}
        />
        <ConvexTextField
          label="Role"
          value={item.role}
